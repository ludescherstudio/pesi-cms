...
// Nur Session-IDs annehmen, die der Server selbst vergeben hat.
...
// _pesi_param() statt (string)-Cast: `pesi_csrf[]=x` kommt als Array an, der
// Cast warnt und legt bei display_errors den Installationspfad offen.
$csrfOk = hash_equals($csrf, _pesi_param($_POST, 'pesi_csrf'));
...
if (!empty($_SESSION[$sk])) {
    $now = time();
    $expired = _pesi_session_expired($_SESSION, $now, $storedPassword,
        defined('PESI_SESSION_IDLE') ? (int)PESI_SESSION_IDLE : null,
        defined('PESI_SESSION_MAX') ? (int)PESI_SESSION_MAX : null);
    if ($expired) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['pesi_login'])) {
    if (!$csrfOk) {
        // Ohne gültiges Token weder Passwort prüfen noch die Bremse verbuchen:
        // Sonst sperrt ein fremdes Formular über den Browser der Kundin deren
        // IP. Das neu ausgelieferte Formular trägt wieder ein gültiges Token.
        $loginError = $t['login_expired'];
    } elseif (_pesi_session_throttle_check() > 0) {
        $loginError = $t['wrong_password'];
    } else {
        // IP-gebundene Bremse — greift auch, wenn ein Angreifer die
        // Session-Cookies verwirft. Prüfen und Reservieren des Versuchs
        // geschehen unter einem Lock, parallele Anfragen kommen darum nicht
        // gemeinsam durch. Die Passwortprüfung läuft danach ohne Lock.
        $wait = _pesi_throttle_acquire();
        if ($wait === null) {
            // Register nicht nutzbar: Ohne IP-Bremse bliebe nur die Session-
            // Bremse, und die umgeht ein neues Cookie. Also niemanden anmelden.
            error_log('pesi: login throttle register unavailable (T15)');
            $loginError = $t['login_unavailable'];
        } elseif ($wait > 0) {
            ...
            if ($ok) {
                ...
                // Reservierten Versuch und Zähler bei Erfolg zurücksetzen
                _pesi_throttle_reset();
                _pesi_session_throttle_reset();
                header('Location: ' . $selfUrl);
                exit;
            }
            // Der IP-Fehlversuch ist mit der Reservierung schon verbucht.
...
if ($auth) {
    $reqPage = _pesi_param($_GET, 'page');
    if ($reqPage !== '' && isset($PESI_PAGES[$reqPage])) {
        $page = $reqPage;
    ...
    // bei jedem Absenden mit, auch bei Strukturaktionen. Mehrere
    // Strukturparameter zugleich sendet kein Button; eine solche Anfrage wird
    // vor jedem Seiteneffekt abgelehnt, statt nacheinander zu schreiben.
    $structCount = (int)isset($_POST['pesi_block']) + (int)isset($_POST['pesi_toggle'])
                 + (int)isset($_POST['pesi_restore']);
    $structural = $structCount > 0;
    $postedHash = _pesi_param($_POST, 'pesi_hash');
    $draft = [];          // eingereichte, nicht gespeicherte Feldwerte
    $invalidIds = [];     // Felder, an denen das Speichern scheiterte
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && $page && $structCount > 1) {
        $msg = $t['err_ambiguous'];
        $msgType = 'error';
    }
    // Block-Operation (Duplizieren / Löschen / Sortieren)
    elseif ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['pesi_block']) && $page) {
        if (!$csrfOk) {
            $msg = $t['err_session'];
            $msgType = 'error';
    ...
    elseif ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['pesi_toggle']) && $page) {
        if (!$csrfOk) {
            $msg = $t['err_session'];
            $msgType = 'error';
        } elseif (!_pesi_hash_matches($basePath . '/' . $page, $postedHash)) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['pesi_save']) && $page && !$structural) {
            ...
            if (!_pesi_hash_matches($fp, $postedHash)) {
                ...
                if (!empty($up['errors'])) {
                    ...
                    foreach ($fields as $id => $field) {
                        $key = 'pesi_field_' . $id;
                        if (($field['type'] ?? '') === 'image' && is_string($up['post'][$key] ?? null)
                            && $up['post'][$key] !== (string)($field['value'] ?? '')) {
        ...
        // Abgelehntes Speichern darf den Entwurf nicht verwerfen: Wer einen
        // langen Text schreibt und sich bei einem Link vertippt, soll nur den
        // Link korrigieren müssen. Hochgeladene Dateien kann kein Browser
        // zurückbekommen; alle Textwerte schon.
        if ($msgType === 'error') {
            $draft = _pesi_draft($fields, $_POST);
        }
    }
}
...
function _pesi_parse(string $file): array {
    ...
    if (!$src) return [];
    $f = [];
    foreach (_pesi_scan($src)['calls'] as $c) {
        if ($c['error'] !== '' || isset($f[$c['id']])) continue;
        $f[$c['id']] = ['id' => $c['id'], 'value' => $c['value'], 'type' => $c['type'], 'label' => $c['label']];
    }
    return $f;
}

/**
 * Findet pesi()-Aufrufe über den PHP-Tokenizer, nicht per Regex im Rohtext.
 * Nur ein echter globaler Funktionsaufruf zählt: kein ->pesi(), ::pesi(),
 * notpesi(), keine Kommentare und kein Text in Strings, Nowdocs oder HTML —
 * sonst würde auch ein Code-Beispiel im gespeicherten Richtext zum Feld.
 *
 * Rückgabe: ['ok' => Tokenizer verfügbar, 'calls' => [...]] in
 * Quelltextreihenfolge. Jeder Aufruf: id, value, type, label, error
 * ('' | 'syntax' | 'type'), und die Byte-Spannen von ID- und Wert-Literal
 * (idAt/idLen, valAt/valLen) für die Ersetzung.
 */
function _pesi_scan(string $src): array {
    if (!function_exists('token_get_all')) return ['ok' => false, 'calls' => []];
    try {
        $raw = token_get_all($src);
    } catch (Throwable $e) {
        return ['ok' => true, 'calls' => []];
    }
    // Einheitliche Liste [id, text, offset]; Leerraum und Kommentare zählen
    // für die Aufrufstruktur nicht.
    $tok = [];
    $pos = 0;
    foreach ($raw as $r) {
        $text = is_array($r) ? $r[1] : $r;
        $tok[] = [is_array($r) ? $r[0] : $r, $text, $pos];
        $pos += strlen($text);
    }
    $skip = [T_WHITESPACE, T_COMMENT, T_DOC_COMMENT];
    $n = count($tok);
    $next = function (int $i) use ($tok, $n, $skip): int {
        for ($i++; $i < $n && in_array($tok[$i][0], $skip, true); $i++);
        return $i;
    };
    $prev = function (int $i) use ($tok, $skip): int {
        for ($i--; $i >= 0 && in_array($tok[$i][0], $skip, true); $i--);
        return $i;
    };
    // Konstantes String-Literal → [Wert, Form] oder null. Doppelte
    // Anführungszeichen nur ohne Escapes/Variablen: dort interpoliert PHP.
    $lit = function (array $t): ?array {
        if ($t[0] !== T_CONSTANT_ENCAPSED_STRING) return null;
        $s = $t[1];
        if ($s[0] === "'") return [str_replace(["\\'", "\\\\"], ["'", "\\"], substr($s, 1, -1)), 'single'];
        $inner = substr($s, 1, -1);
        return (strpos($inner, '\\') === false && strpos($inner, '$') === false) ? [$inner, 'double'] : null;
    };
    $notCall = [T_OBJECT_OPERATOR, T_DOUBLE_COLON, T_FUNCTION, T_NEW, T_CONST];
    if (defined('T_NULLSAFE_OBJECT_OPERATOR')) $notCall[] = T_NULLSAFE_OBJECT_OPERATOR;

    $calls = [];
    for ($i = 0; $i < $n; $i++) {
        $isName = ($tok[$i][0] === T_STRING && $tok[$i][1] === 'pesi')
            || (defined('T_NAME_FULLY_QUALIFIED') && $tok[$i][0] === T_NAME_FULLY_QUALIFIED && $tok[$i][1] === '\\pesi');
        if (!$isName) continue;
        $p = $prev($i);
        if ($p >= 0 && in_array($tok[$p][0], $notCall, true)) continue;
        $j = $next($i);
        if ($j >= $n || $tok[$j][1] !== '(') continue;

        $call = ['id' => '', 'value' => '', 'type' => 'text', 'label' => '', 'error' => 'syntax',
                 'idAt' => 0, 'idLen' => 0, 'valAt' => 0, 'valLen' => 0, 'pos' => $tok[$i][2]];
        $calls[] = &$call;
        // 1. ID
        $j = $next($j);
        $id = $j < $n ? $lit($tok[$j]) : null;
        if ($id === null) { unset($call); continue; }
        $call['id'] = $id[0];
        if (!preg_match('/^[a-zA-Z0-9_]+$/', $id[0])) { unset($call); continue; }
        $call['idAt'] = $tok[$j][2];
        $call['idLen'] = strlen($tok[$j][1]);
        $j = $next($j);
        if ($j >= $n || $tok[$j][1] !== ',') { unset($call); continue; }
        // 2. Wert: einfach zitiert oder Nowdoc <<<'PESI' mit unverrücktem Endmarker
        $j = $next($j);
        if ($j >= $n) { unset($call); continue; }
        if ($tok[$j][0] === T_CONSTANT_ENCAPSED_STRING && $tok[$j][1][0] === "'") {
            $call['value'] = $lit($tok[$j])[0];
            $call['valAt'] = $tok[$j][2];
            $call['valLen'] = strlen($tok[$j][1]);
        } elseif ($tok[$j][0] === T_START_HEREDOC && preg_match("/^<<<[ \\t]*'PESI'\\r?\\n$/", $tok[$j][1])) {
            $start = $j;
            $body = '';
            $k = $j + 1;
            if ($k < $n && $tok[$k][0] === T_ENCAPSED_AND_WHITESPACE) { $body = $tok[$k][1]; $k++; }
            if ($k >= $n || $tok[$k][0] !== T_END_HEREDOC || $tok[$k][1] !== 'PESI') { unset($call); continue; }
            $call['value'] = (string)preg_replace('/\r?\n\z/', '', $body);
            $call['valAt'] = $tok[$start][2];
            $call['valLen'] = $tok[$k][2] + strlen($tok[$k][1]) - $tok[$start][2];
            $j = $k;
        } else {
            unset($call);
            continue;
        }
        // 3./4. Typ und Label, beide optional; ein abschliessendes Komma ist erlaubt
        $j = $next($j);
        foreach (['type', 'label'] as $slot) {
            if ($j >= $n || $tok[$j][1] !== ',') break;
            $k = $next($j);
            if ($k < $n && $tok[$k][1] === ')') { $j = $k; break; }
            $v = $k < $n ? $lit($tok[$k]) : null;
            if ($v === null) { $j = -1; break; }
            $call[$slot] = $v[0];
            $j = $next($k);
        }
        if ($j >= 0 && $j < $n && $tok[$j][1] === ',') $j = $next($j);
        if ($j < 0 || $j >= $n || $tok[$j][1] !== ')') { unset($call); continue; }
        $call['error'] = in_array($call['type'], _pesi_types(), true) ? '' : 'type';
        unset($call);
    }
...
function _pesi_unparsed_fields(string $file): array {
    ...
    $out = [];
    foreach (_pesi_scan($src)['calls'] as $c) {
        if ($c['error'] !== '') $out[] = $c['id'] !== '' ? $c['id'] : '?';
    }
    return array_values(array_unique($out));
}

// IDs, die in einer Seite mehr als einmal vorkommen — Code S7. Welche Stelle
// das Dashboard dann bearbeiten würde, hinge vom Zufall der Schreibweise ab;
// darum wird eine solche Seite gar nicht gespeichert.
function _pesi_duplicate_ids(string $src): array {
    $seen = [];
    foreach (_pesi_scan($src)['calls'] as $c) {
        if ($c['id'] !== '') $seen[$c['id']] = ($seen[$c['id']] ?? 0) + 1;
    }
...
function _pesi_backup(string $file): bool {
    $st = _pesi_backup_begin($file);
    if ($st === false) return false;
    _pesi_backup_finish($st);
    return true;
}
...
function _pesi_commit(string $file, string $mod, ?string $expectedHash = null): ?array {
    ...
    // Ungültiges PHP erreicht die Live-Datei nie. Ist die Prüfung eingeschaltet,
    // aber nicht ausführbar, wird ebenfalls nicht veröffentlicht — mit eigener
    // Meldung, denn ein Syntaxfehler ist dann nicht festgestellt. Ungeprüft
    ...
    // nicht mit dem inzwischen veralteten Snapshot überschreiben. Die neue
    // Sicherung 1 muss genau diesen Snapshot enthalten.
    if (!_pesi_hash_matches($file, $currentHash)) {
        $finish($lock, $tmp);
        return ['msg' => $t['err_stale'], 'type' => 'error'];
    }
    $bk = _pesi_backup_begin($file, $currentHash);
    if ($bk === false) {
        // Scheitert die Kopie, weil die Datei inzwischen fremd geändert wurde,
        // ist das ein veralteter Stand und kein Problem der Sicherung.
        $stale = !_pesi_hash_matches($file, $currentHash);
        $finish($lock, $tmp);
        return ['msg' => $t[$stale ? 'err_stale' : 'err_backup'], 'type' => 'error'];
    }
    // Letzte Prüfung unmittelbar vor dem Austausch. Ein FTP-Server kennt den
    // pesi-Lock nicht; das verbleibende Fenster bis zum rename() ist kurz,
    // aber nicht null (README, Limitations).
    if (!_pesi_hash_matches($file, $currentHash)) {
        _pesi_backup_rollback($bk);
        $finish($lock, $tmp);
        return ['msg' => $t['err_stale'], 'type' => 'error'];
    }
    if (!@rename($tmp, $file)) {
...
function _pesi_save(string $file, array $fields, array $post, ?string $expectedHash = null): array {
    ...
    if (_pesi_duplicate_ids($src)) return ['msg' => $t['err_dup_ids'], 'type' => 'error'];
    ...
    foreach ($fields as $id => $fld) {
        $k = 'pesi_field_' . $id;
        if (!isset($post[$k]) || !is_string($post[$k])) continue;
        ...
        $nv = str_replace(["\r\n", "\r"], "\n", (string)$post[$k]);
        // Ohne DOM bliebe vom Richtext nur Klartext. Das Dashboard bietet ihn
        // dann nicht zum Bearbeiten an (T17); hier wird er nie umgeschrieben.
        ...
        if (_pesi_has_marker($nv)) { $blocked[] = $fld['label'] ?: $id; $invalidIds[] = $id; continue; }
        $mod = _pesi_replace($mod, $id, $nv);
        $n++;
    }
    // Vor dem ersten Schreibzugriff abbrechen — ein Save ist ganz oder gar nicht.
    if ($blocked) return ['msg' => sprintf($t['err_marker'], implode(', ', $blocked)), 'type' => 'error', 'invalid' => $invalidIds];
    if ($invalid) return ['msg' => sprintf($t['err_invalid_fields'], implode(', ', $invalid)), 'type' => 'error', 'invalid' => $invalidIds];
...
// Eingereichte Werte, die vom gespeicherten Stand abweichen: id => Wert.
// Für die erneute Anzeige nach abgelehntem Speichern. Richtext wird wie beim
// Speichern bereinigt; alles andere escaped die Ausgabe.
function _pesi_draft(array $fields, array $post): array {
    $draft = [];
    foreach ($fields as $id => $fld) {
        $k = 'pesi_field_' . $id;
        if (!isset($post[$k]) || !is_string($post[$k])) continue;
        $v = str_replace(["\r\n", "\r"], "\n", $post[$k]);
        if (($fld['type'] ?? '') === 'richtext' && function_exists('_pesi_sanitize_html')) {
            $v = _pesi_sanitize_html($v);
        }
        if ($v !== (string)($fld['value'] ?? '')) $draft[$id] = $v;
    }
    return $draft;
}
 ...
 */
/**
 * Verschachtelte pesi:item-Spannen — nicht unterstützt. Der non-greedy Parser
 * beendet den äusseren Block am ersten inneren End-Marker; Duplizieren
 * schriebe halbe Blöcke und doppelte Feld-IDs, und zwar `php -l`-sauber.
 */
function _pesi_block_nested(string $file): bool {
    $src = is_file($file) ? (string)file_get_contents($file) : '';
    if (!preg_match_all('/<!--\s*(\/?)\s*pesi:item(?:\s+[a-z0-9_]+:\d+)?\s*-->/i', $src, $m, PREG_SET_ORDER)) {
        return false;
    }
    $depth = 0;
    foreach ($m as $x) {
...
function _pesi_block_op(string $file, string $group, int $inst, string $action, ?string $expectedHash = null): array {
    ...
    if (_pesi_block_nested($file)) return ['msg' => $t['blk_nested'], 'type' => 'error'];
    ...
    if ($action === 'dup' || $action === 'add') {
        ...
        $clone   = $srcBlk['text'];
        // Feld-ID-Präfix '<group>_<inst>_' umschreiben — nur in echten
        // pesi()-Aufrufen, über deren Token-Spannen. Ein Regex im Rohtext
        // träfe auch ->pesi(), Kommentare und Beispieltext im Richtext.
        $oldPre = $group . '_' . $srcBlk['inst'] . '_';
        $newPre = $group . '_' . $newInst . '_';
        $end    = $srcBlk['offset'] + strlen($clone);
        foreach (array_reverse(_pesi_scan($src)['calls']) as $c) {
            if ($c['idLen'] === 0 || $c['idAt'] < $srcBlk['offset'] || $c['idAt'] >= $end) continue;
            if (strncmp($c['id'], $oldPre, strlen($oldPre)) !== 0) continue;
            $q = $src[$c['idAt']];
            $clone = substr_replace($clone, $q . $newPre . substr($c['id'], strlen($oldPre)) . $q,
                $c['idAt'] - $srcBlk['offset'], $c['idLen']);
        }
        if (substr($clone, -1) !== "\n") $clone .= "\n";
...
function _pesi_replace(string $src, string $id, string $nv): string {
    // Ersetzungswert einmal aufbauen und per substr_replace() einsetzen. NIE
    // über den Replacement-String von preg_replace(): dort würden $1/${1}/\1
    // im Nutzertext als Backreferences interpretiert (abgelehnter Commit bzw.
    // stille Content-Korruption bei z. B. Preisen wie „$1.000").
    $repl = _pesi_hd($nv)
        ? "<<<'PESI'\n" . $nv . "\nPESI"
        : "'" . _pesi_esc($nv) . "'";
    // Genau die Wertspanne des einen gültigen Aufrufs mit dieser ID. Bei
    // doppelter ID nichts anfassen (_pesi_save lehnt vorher mit S7 ab).
    $hit = null;
    foreach (_pesi_scan($src)['calls'] as $c) {
        if ($c['id'] !== $id) continue;
        if ($hit !== null || $c['error'] !== '') return $src;
        $hit = $c;
    }
    if ($hit === null) return $src;
    return substr_replace($src, $repl, $hit['valAt'], $hit['valLen']);
}
...
// Zweite, sitzungsgebundene Bremse für Versuche aus derselben Session.
// Die Umgehung per neuem Cookie fängt das IP-Register ab.
...
function _pesi_session_throttle_fail(): void {
    ...
    $_SESSION['pesi_fail_until'] = time() + 2 ** min(8, $fails);
}
...
// Liest und ändert das Register unter stabilem Sidecar-Lock und tauscht die
// Datei erst vollständig geschrieben aus. Ein Prozessabbruch darf die Bremse
// nicht durch abgeschnittenes JSON unbemerkt zurücksetzen. $mutate gibt den
// neuen Stand zurück oder null, wenn nichts zu schreiben ist.
// Ergebnis: false, wenn das Register nicht gelesen oder geschrieben werden
// konnte — der Aufrufer darf das nicht als „frei“ deuten.
function _pesi_throttle_write(callable $mutate): bool {
    $lock = @fopen(_pesi_throttle_lock_file(), 'c+');
    if (!$lock) return false;
    if (!flock($lock, LOCK_EX)) { fclose($lock); return false; }
    $f = _pesi_throttle_file();
    $raw = is_file($f) ? @file_get_contents($f) : '';
    if ($raw === false) { flock($lock, LOCK_UN); fclose($lock); return false; }
    // Ungültiges JSON kann durch den atomaren Austausch nicht entstehen, nur
    // durch Eingriffe von Hand. Es wird mit dem nächsten Stand überschrieben.
    ...
    if ($data === null) { flock($lock, LOCK_UN); fclose($lock); return true; }
...
// Prüft die Sperre und reserviert im selben Lock einen Versuch.
// Rückgabe: 0 = Versuch reserviert, > 0 = gesperrt für so viele Sekunden,
// null = Register nicht nutzbar.
// Die Reservierung wird vorab als Fehlversuch verbucht (Backoff 2, 4, 8 …
// Sekunden, bei 2^8 = 256 s eingefroren). Parallele Anfragen derselben IP
// sehen sie sofort und werden abgewiesen; ein erfolgreicher Login löscht sie
// über _pesi_throttle_reset().
function _pesi_throttle_acquire(): ?int {
    $wait = 0;
    $ok = _pesi_throttle_write(function (array $data, int $now) use (&$wait): ?array {
        $key   = _pesi_throttle_key();
        $until = (int)($data[$key]['until'] ?? 0);
        if ($until > $now) {
            $wait = $until - $now;
            return null;
        }
        $n = min(20, (int)($data[$key]['n'] ?? 0) + 1);
        $data[$key] = ['n' => $n, 'until' => $now + 2 ** min($n, 8)];
        return $data;
    });
    return $ok ? $wait : null;
}
...
// Skalarer Request-Parameter. PHP liefert `name[]=x` als Array; ein
// (string)-Cast darauf warnt, ein Array als Schlüssel wirft einen TypeError.
// Beides legt bei display_errors Pfade offen. Nicht-Strings zählen als leer.
function _pesi_param(array $src, string $key): string {
    return isset($src[$key]) && is_string($src[$key]) ? $src[$key] : '';
}

// Ist die angemeldete Sitzung abgelaufen? Inaktivität, absolute Höchstdauer
// und Passwortwechsel beenden sie. Beide Zeitgrenzen gelten unabhängig
// voneinander, jeweils mit mindestens einer Minute; eine absolute Grenze unter
// der Inaktivitätsgrenze greift also auch früher.
function _pesi_session_expired(array $s, int $now, string $storedPassword, ?int $idleCfg, ?int $maxCfg): bool {
    ...
    return $now - (int)($s['pesi_last'] ?? 0) > $idle
        ...
        || !hash_equals(hash('sha256', $storedPassword), (string)($s['pesi_pw_fingerprint'] ?? ''));
}
...
function _pesi_handle_uploads(array $fields, array $files, array $post, string $basePath): array {
    ...
    foreach ($fields as $id => $fld) {
        ...
        // move_uploaded_file() überschreibt ein vorhandenes Ziel. 48 Bit Zufall
        // plus Existenzprüfung, damit nie ein fremdes Bild ersetzt wird.
        $dest = '';
        for ($try = 0; $try < 5 && $dest === ''; $try++) {
            $fname = _pesi_slug($f['name']) . '-' . bin2hex(random_bytes(6)) . '.' . $ext;
            if (!file_exists($realDir . '/' . $fname)) $dest = $realDir . '/' . $fname;
        }
        if ($dest === '' || !move_uploaded_file($f['tmp_name'], $dest)) {
            $errors[] = sprintf($t['up_err_failed'], $fld['label'] ?: $id);
            continue;
        }
        @chmod($dest, 0644);
...
function _pesi_cleanup_old(string $basePath, array $old, array $pages): void {
    ...
    // Die Sidecar-Locks aller registrierten Seiten bleiben geteilt gehalten,
    // bis gelöscht ist. Ein Commit, der ein Bild wieder einsetzt, ist damit
    // entweder schon im Scan sichtbar oder wartet, bis die Bereinigung fertig
    // ist. Ein Commit hält immer nur einen Lock, darum kein Deadlock.
    $locks = [];
    $release = static function () use (&$locks): void {
        foreach ($locks as $l) { flock($l, LOCK_UN); fclose($l); }
        $locks = [];
    };

    // Alle aktuell referenzierten Pfade über sämtliche Seiten sammeln. Ist ein
    // vorhandener Stand nicht lesbar, ist der Bestand unvollständig — dann
    // lieber ein verwaistes Bild behalten als ein verwendetes löschen.
    $inUse = [];
    $raw   = '';
    foreach (array_keys($pages) as $pg) {
        ...
        foreach ($versions as $version) {
            $src = is_file($version) ? @file_get_contents($version) : false;
            if ($src === false) { $release(); return; }
            $raw .= $src;
    ...
    foreach (array_keys($cands) as $rel) {
...
function _pesi_strings(): array { return [
    'de' => [
        ...
        'err_no_lint'       => 'Diese Änderung wurde nicht übernommen, weil der Server sie gerade nicht technisch prüfen kann. Ihre Seite ist unverändert geblieben. Bitte melden Sie sich bei Ihrer Website-Betreuung. (Code T7)',
    ...
    'en' => [
        ...
        'err_no_lint'       => 'This change was not applied because the server cannot run its technical check right now. Your page is unchanged. Please contact whoever looks after your website. (Code T7)',
...
// Richtext braucht ext/dom: ohne sie bliebe nach dem Speichern nur Klartext.
// Dann kein Editor, sondern eine Leseansicht mit Hinweis (T17).
$rtEditable = class_exists('DOMDocument');
$hasRt = false;
foreach ($fields as $f) { if ($f['type'] === 'richtext' && $rtEditable) { $hasRt = true; break; } }
?>
<!DOCTYPE html>
<html lang="<?=$lang?>">
<head>
...
.fc-err,.fc-err:focus-within{border-color:var(--er);box-shadow:0 0 0 3px var(--er-b)}
.rt-ro{height:auto;min-height:0;white-space:normal;color:var(--tx2)}
...
.img-drop:hover,.img-drop.drag,.img-drop:focus-within{border-color:var(--b);background:var(--b-s);color:var(--tx)}
  ...
  <div class="M">
    <?php
      ...
      if ($page) {
          ...
          $pageSrc = is_file($basePath . '/' . $page) ? (string)file_get_contents($basePath . '/' . $page) : '';
          $dupIds = _pesi_duplicate_ids($pageSrc);
          if ($dupIds) $diag[] = sprintf($t['warn_dup_ids'], $page, implode(', ', $dupIds));
          if (!$rtEditable && array_filter($fields, fn($f) => $f['type'] === 'richtext')) {
              $diag[] = $t['warn_no_dom'];
          }
      }
    ...
    <?php if ($page): ?>
      ...
      <form method="POST" id="pf" enctype="multipart/form-data" class="ws-edit">
        ...
        <input type="hidden" name="pesi_csrf" value="<?=htmlspecialchars($csrf)?>">
        <input type="hidden" name="pesi_hash" value="<?=htmlspecialchars($draft ? $postedHash : (string)$fileHash)?>">
        <?php /* Enter in einem Textfeld löst den ersten Submit-Button aus. Ohne diesen
           hier wäre das der erste Ein-/Ausblenden- oder Blockknopf. Nur visuell
           versteckt (.sr, nicht hidden), sonst zählt er nicht als Standardaktion. */ ?>
        <?php if (!empty($fields)): ?><button type="submit" class="sr sv-def" tabindex="-1" aria-hidden="true"><?=htmlspecialchars($t['save_btn'])?></button><?php endif; ?>
        ...
        <div class="cnt-wrap">
          ...
          <?php if ($draft): ?><div class="ms info"><span><?=htmlspecialchars($t['draft_kept'])?></span></div><?php endif; ?>
          <?php if ($isGlobals): ?><div class="ms info"><span><?=htmlspecialchars($t['globals_hint'])?></span></div><?php endif; ?>

          <?php
            $toggles     = is_file($pageAbs) ? _pesi_toggle_parse($pageAbs) : [];
          ...
          <?php else: ?>
            <?php
              ...
              // Verschachtelte Einträge: Felder bleiben bearbeitbar, aber ohne
              // Blockknöpfe — die Engine würde sie ohnehin ablehnen (S6).
              $blkNested = _pesi_block_nested($basePath . '/' . $page);
              $blocks  = $blkNested ? [] : _pesi_block_parse($basePath . '/' . $page);
              if ($blkNested) {
                  echo '<div class="ms error"><span>' . htmlspecialchars($t['blk_nested']) . '</span></div>';
              }
            ...
            <?php endif; $curBlk = $bk; endif; ?>
              <?php
                ...
                // Nach abgelehntem Speichern: eingereichter Wert statt Dateiwert.
                // data-saved hält den gespeicherten Stand, damit die Änderung
                // weiter als ungespeichert zählt.
                $isDraft = array_key_exists($id, $draft);
                $val = $isDraft ? $draft[$id] : (string)$fld['value'];
                $bad = in_array($id, $invalidIds, true);
                $fx = ($isDraft ? ' data-saved="' . htmlspecialchars((string)$fld['value']) . '"' : '') . ($bad ? ' aria-invalid="true"' : '');
                $ft  = $fieldToggle[$id] ?? null;
              ?>
              <div class="fc<?=$ft && !$ft['visible'] ? ' fc-off' : ''?><?=$bad ? ' fc-err' : ''?>">
                ...
                <?php if ($fld['type'] === 'text'): ?>
                  <input type="text" id="<?=$fid?>" name="pesi_field_<?=htmlspecialchars($id)?>" value="<?=htmlspecialchars($val)?>"<?=$fx?> class="fi">

                <?php elseif ($fld['type'] === 'url'): ?>
                  <input type="text" inputmode="url" autocomplete="url" id="<?=$fid?>" name="pesi_field_<?=htmlspecialchars($id)?>" value="<?=htmlspecialchars($val)?>"<?=$fx?> class="fi" placeholder="<?=htmlspecialchars($t['url_ph'])?>">

                <?php elseif ($fld['type'] === 'email'): ?>
                  <input type="email" inputmode="email" autocomplete="email" id="<?=$fid?>" name="pesi_field_<?=htmlspecialchars($id)?>" value="<?=htmlspecialchars($val)?>"<?=$fx?> class="fi">

                <?php elseif ($fld['type'] === 'tel'): ?>
                  <input type="tel" inputmode="tel" autocomplete="tel" id="<?=$fid?>" name="pesi_field_<?=htmlspecialchars($id)?>" value="<?=htmlspecialchars($val)?>"<?=$fx?> class="fi">
                ...
                <?php elseif ($fld['type'] === 'richtext' && !$rtEditable): ?>
                  <div class="fi rt-ro"><?=_pesi_sanitize_html($val)?></div>
                  <p class="img-hint"><?=htmlspecialchars($t['rt_no_dom'])?></p>
...
(function(){
  ...
  var snap=new Map();
  // Ausgangswert ist der gespeicherte Stand. Nach abgelehntem Speichern steht
  // der Entwurf im Feld und der gespeicherte Wert in data-saved — der Entwurf
  // zählt so weiter als ungespeichert und die Warnung vor dem Verlassen greift.
  form.querySelectorAll('input,textarea,select').forEach(function(el){
    snap.set(el, el.type==='file' ? 0 : (el.hasAttribute('data-saved') ? el.getAttribute('data-saved') : el.value));
  });
  var rtDirty=new Set();
  ...
  function count(){
    ...
    snap.forEach(function(init,el){
      // Richtext-Entwurf, der danach im Editor weiterbearbeitet wurde: zählt
      // schon über rtDirty, nicht ein zweites Mal über die versteckte Textarea.
      if(el.id && el.id.indexOf('h_')===0 && rtDirty.has(el.id.slice(2))) return;
  ...
  // Enter in einem einzeiligen Feld heisst Speichern, nie eine Strukturaktion.
  // Der versteckte Standardknopf regelt das schon nach HTML-Spezifikation;
  // hier zusätzlich ausdrücklich, unabhängig von Browser-Eigenheiten.
  // Textareas, Quill und fokussierte Knöpfe behalten ihr eigenes Enter.
  form.addEventListener('keydown',function(e){
    var el=e.target;
    if(e.key!=='Enter'||e.isComposing||e.defaultPrevented) return;
    if(el.tagName!=='INPUT'||['text','url','email','tel','search','number'].indexOf(el.type)<0) return;
    var def=form.querySelector('button.sv-def');
    if(!def) return;
    e.preventDefault();
    def.click();
  });
  refresh();
  // Nach abgelehntem Speichern direkt zum ersten markierten Feld.
  var bad=form.querySelector('input[aria-invalid="true"],textarea[aria-invalid="true"]:not([style*="display:none"])');
  if(bad) bad.focus();
})();
</script>
...
<script>
// Prototyplos: eine erlaubte Feld-ID wie __proto__ würde bei {} den Prototyp
// setzen statt einen Eintrag anzulegen, und der Editor fiele beim Speichern weg.
